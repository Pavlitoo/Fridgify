...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _openPremiumScreen() async {
    bool? bought = await Navigator.push(context, MaterialPageRoute(builder: (_) => const PremiumScreen()));
    if (bought == true) _checkPremiumStatus();
  }

  Future<void> _resetPremiumForTesting() async {
    await SubscriptionService().debugResetPremium();
    _checkPremiumStatus();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Premium Reset"), backgroundColor: Colors.orange));
  }
  ...
  Future<void> _openMyLocation() async {
    ...
    if (!serviceEnabled) { if(mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('GPS вимкнено'), backgroundColor: Colors.orange)); return; }
    LocationPermission permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied) permission = await Geolocator.requestPermission();
    if (permission == LocationPermission.denied) return;

    if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppText.get('searching_loc')), backgroundColor: Colors.blue));
              ...
              child: Column(children: [
                ...
                if (!_isPremium)
                  Container(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text("Купити Premium", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black)),
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), elevation: 6),
                    ),
                  ),
                if (_isPremium) TextButton(onPressed: _resetPremiumForTesting, child: const Text("🔴 Скинути Premium (Dev)", style: TextStyle(color: Colors.red))),
                ...
                Container(margin: const EdgeInsets.fromLTRB(16, 20, 16, 40), padding: const EdgeInsets.symmetric(vertical: 8), decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(24), border: Border.all(color: dividerColor.withOpacity(0.05)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 20, offset: const Offset(0, 5))]), child: Column(children: [
                  ...
                  ListTile(
                      ...
                      onTap: () { if (!_isPremium) { _openPremiumScreen(); } else { Navigator.push(context, MaterialPageRoute(builder: (context) => const FamilyScreen())); } }
                  ),
                  const Divider(height: 1),
